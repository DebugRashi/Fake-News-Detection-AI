...
### Enhanced Python experiments

The file `enhanced_liar_experiments.py` adds several research-style extensions on top of the original fake-news detector:

* **Explainable fake news detection:** predicts `TRUE` / `FALSE` and writes `enhanced_outputs/sample_explanation.csv`, showing the words or phrases that pushed the model toward each class. It uses LIME and SHAP when those libraries are installed, and also includes a built-in Logistic Regression coefficient explanation fallback.
* **Imbalance-aware fake news detection:** compares a normal text model with a `class_weight="balanced"` model so you can discuss how class balancing changes precision, recall, F1 score, and the confusion matrix.
* **Uncertainty-aware detection:** writes `enhanced_outputs/uncertainty_rejection.csv`, where low-confidence predictions are rejected instead of always forcing a label.
* **Hybrid model:** trains a second model that combines statement text with LIAR speaker/context features such as speaker, party, subject, state, context, and previous truth-history counts.
* **Error analysis:** writes `enhanced_outputs/misclassified_errors.csv` and `enhanced_outputs/experiment_report.txt` to show what kinds of claims the model gets wrong.
* **Optional cross-domain robustness:** if you have another fake-news CSV, pass it with `--cross-domain-csv path/to/file.csv` to test how a LIAR-trained model generalizes outside LIAR.
...
If your Python version is very new and scientific packages fail to import, see `SETUP_ENHANCED.md`. Python 3.11 or 3.12 is recommended for these experiments.

To run the enhanced experiments from the project folder:

```
python enhanced_liar_experiments.py
```

Example with a custom explanation statement:

```
python enhanced_liar_experiments.py --explain "The unemployment rate doubled last month."
```
